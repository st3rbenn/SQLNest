/**
 * Client fetch pour `GET /api/canvas-state/checksum-history`.
 *
...
function endpoint(teamSlug: string | null): string {
	return teamSlug
		? `${window.CONTEXT.apiBaseUrl}/api/teams/${encodeURIComponent(teamSlug)}/canvas-state/checksum-history`
		: `${window.CONTEXT.apiBaseUrl}/api/canvas-state/checksum-history`;
}
